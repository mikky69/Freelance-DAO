...
contract FreeLanceDAOEscrowPayment is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Gets all details for a specific job.
     * @param jobId The ID of the job to retrieve.
     * @return A Job struct containing all job details.
     * @dev Note that Milestone data is not returned here; use getMilestone for that.
     */
    function getJob(uint256 jobId) external view returns (Job memory) {
        Job storage j = jobs[jobId];
        require(j.exists, "Job not exist");
        // We have to copy to a memory struct to return it.
        // Note: this does not copy the milestones array.
        return Job({
            jobId: j.jobId,
            jobType: j.jobType,
            client: j.client,
            ...
            status: j.status,
            funded: j.funded,
            milestones: new Milestone[](0), // Milestones should be fetched separately
            jobTitle: j.jobTitle,
    ...
    /// @notice Returns all jobs created by a specific client.
    ...
    /// @notice Returns all jobs created in the contract.
    function getAllJobs()
        external
        view
        returns (Job[] memory) {
