...
export async function PUT(request: NextRequest) {
  try {
    ...
    let updatedUser
    if (freelancer) {
      updatedUser = await Freelancer.findByIdAndUpdate(userId, updateData, { new: true, runValidators: true }).select(
        "-password",
      )
    } else {
      updatedUser = await Client.findByIdAndUpdate(userId, updateData, { new: true, runValidators: true }).select(
        "-password",
      )
    }
