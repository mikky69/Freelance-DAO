...
import {
  Shield, Clock, CheckCircle, AlertTriangle, Lock, Unlock,
  FileText, Calendar, Briefcase, ArrowRight, ExternalLink,
} from "lucide-react"
...
export default function EscrowPage() {
    ...
    try {
      ...
      const mapped = jobs.map((j: any) => {
        ...
        const milestones = j.milestones.map((m: any, idx: number) => ({
          ...
          amount: `${Number(formatEther(m.amount)).toFixed(4)} ETH`,
          status: m.confirmed ? "Released" : m.delivered ? "Locked" : "Pending",
          date: "—",
          idx,
        }))
        const confirmedCount = j.milestones.filter((m: any) => m.confirmed).length
        const progress = j.milestones.length > 0
          ? Math.round((confirmedCount / j.milestones.length) * 100) : 0
        return {
          ...
          totalAmount: `${total.toFixed(4)} ETH`,
          lockedAmount: `${lockedAmount.toFixed(4)} ETH`,
          releasedAmount: `${confirmedAmt.toFixed(4)} ETH`,
          status: j.status === 2 ? "Completed" : j.funded ? "Active" : "Locked",
          progress,
          milestones,
  ...
  const handleRelease = useCallback(async (jobId: number, idx: number) => {
    ...
    writeContract(
      { address: ESCROW_ADDRESS, abi: ESCROW_ABI, functionName: "confirmMilestone", args: [jobId, idx] },
      {
        onSuccess: async (hash) => {
          toast.success(`Milestone confirmed: ${hash}`)
          await loadJobs()
        },
          ...
          <div className="container mx-auto px-4 py-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <p className="text-slate-600">Secure payments powered by Base blockchain</p>
              </div>
              <div className="flex items-center space-x-3">
                <Badge variant="secondary" className="bg-green-100 text-green-700">
                  <Shield className="w-4 h-4 mr-1" />Blockchain Secured
                </Badge>
                <Button className="bg-blue-500 hover:bg-blue-600">
                  <FileText className="w-4 h-4 mr-2" />View Documentation
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="space-y-4">
              ...
              {escrowContracts.map((escrow, index) => (
                <Card
                  key={escrow.id}
                  className={`cursor-pointer transition-all ${selectedEscrow === index ? "ring-2 ring-blue-500 border-blue-200" : "hover:shadow-md"}`}
                  onClick={() => setSelectedEscrow(index)}
                >
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <Badge variant="outline" className="mb-2 font-mono text-xs">{escrow.id}</Badge>
                        <CardTitle className="text-lg">{escrow.project}</CardTitle>
                      </div>
                      <Badge className={escrow.status === "Active" ? "bg-blue-100 text-blue-700" : escrow.status === "Completed" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}>
                        {escrow.status}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center space-x-2 text-xs text-slate-500">
                        <Calendar className="w-3 h-3" /><span>Due: {escrow.deadline}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <Tabs defaultValue="overview" className="space-y-6">
                ...
                <TabsContent value="overview" className="space-y-6">
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center">
                        <Briefcase className="w-5 h-5 mr-2 text-blue-500" />Project Overview
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <div className="flex items-center space-x-2 text-sm text-slate-600">
                              <Badge variant="outline" className="font-mono">{currentEscrow.id}</Badge>
                              <span>•</span><span>Created {currentEscrow.createdDate}</span>
                            </div>
                          </div>
                              ...
                              <div className="flex items-center space-x-2 mt-1">
                                <Avatar className="w-6 h-6">
                                  <AvatarFallback className="text-xs bg-blue-100 text-blue-600">
                                    {currentEscrow.client?.slice(2, 4).toUpperCase()}
                                  </AvatarFallback>
                                </Avatar>
                                <span className="text-sm font-mono truncate">{currentEscrow.client?.slice(0, 10)}…</span>
                              </div>
                            </div>
                            <div>
                              ...
                              <div className="flex items-center space-x-2 mt-1">
                                <Avatar className="w-6 h-6">
                                  <AvatarFallback className="text-xs bg-green-100 text-green-600">
                                    {currentEscrow.freelancer?.slice(2, 4).toUpperCase()}
                                  </AvatarFallback>
                                </Avatar>
                                <span className="text-sm font-mono truncate">{currentEscrow.freelancer?.slice(0, 10)}…</span>
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center">
                        <Shield className="w-5 h-5 mr-2 text-green-500" />Security Features
                      </CardTitle>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {[
                          { icon: Lock, color: "green", title: "Smart Contract Escrow", desc: "Funds locked in tamper-proof smart contracts" },
                          { icon: CheckCircle, color: "blue", title: "Milestone-Based Release", desc: "Payments released only when work is approved" },
                          { icon: AlertTriangle, color: "purple", title: "Dispute Protection", desc: "Built-in DAO dispute resolution mechanism" },
                        ].map(({ icon: Icon, color, title, desc }) => (
                          <div key={title} className={`text-center p-4 border border-${color}-200 rounded-lg`}>
                ...
                <TabsContent value="milestones" className="space-y-6">
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center">
                        <Calendar className="w-5 h-5 mr-2 text-purple-500" />Project Milestones
                      </CardTitle>
                      <CardDescription>Track progress and payment releases</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        {currentEscrow.milestones.map((milestone: any, index: number) => (
                          <div key={index} className="flex items-center space-x-4 p-4 border border-slate-200 rounded-lg">
                            ...
                            <div className="text-right">
                              ...
                              <Badge variant="secondary" className={milestone.status === "Released" ? "bg-green-100 text-green-700" : milestone.status === "Locked" ? "bg-yellow-100 text-yellow-700" : "bg-slate-100 text-slate-700"}>
                                {milestone.status}
                              </Badge>
                            </div>
                            {milestone.status === "Locked" && (
                              <Button size="sm" className="bg-blue-500 hover:bg-blue-600" disabled={txPending} onClick={() => handleRelease(currentEscrow.jobId, milestone.idx)}>
                ...
                <TabsContent value="contract" className="space-y-6">
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center">
                        <FileText className="w-5 h-5 mr-2 text-blue-500" />Smart Contract Details
                      </CardTitle>
                    ...
                    <CardContent className="space-y-4">
                      <Alert>
                        ...
                        <AlertDescription>
                          This escrow is secured by a UUPS-upgradeable smart contract on the Base blockchain. All transactions are immutable and transparent.
                        </AlertDescription>
                      </Alert>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-3">
                          <div>
                            ...
                            <div className="flex items-center space-x-2 mt-1">
                              <code className="text-xs bg-slate-100 px-2 py-1 rounded font-mono break-all">{ESCROW_ADDRESS}</code>
                              <Button variant="ghost" size="sm" onClick={() => window.open(`https://sepolia.basescan.org/address/${ESCROW_ADDRESS}`, "_blank")}>
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-600 mt-1">Base Sepolia Testnet</p>
                          </div>
                          <div>
                            <span className="text-sm font-medium text-slate-700">Contract Type</span>
                            <p className="text-sm text-slate-600 mt-1">Milestone-based Escrow (UUPS)</p>
                          </div>
                        </div>
                        <div className="space-y-3">
                          <div>
                      ...
                      <div className="flex gap-2 pt-4">
                        <Button variant="outline" onClick={() => window.open(`https://sepolia.basescan.org/address/${ESCROW_ADDRESS}`, "_blank")}>
                ...
                <TabsContent value="history" className="space-y-6">
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center">
                        <Clock className="w-5 h-5 mr-2 text-purple-500" />Transaction History
                      </CardTitle>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        {[
                          { type: "Contract Created", amount: currentEscrow.totalAmount, date: currentEscrow.createdDate, txHash: "0x1a2b…", status: "Confirmed" },
                          { type: "Milestone Released", amount: "0.07 ETH", date: "Jan 1, 2025", txHash: "0x2b3c…", status: "Confirmed" },
                        ].map((tx, index) => (
